...
- **Alphanumerics and hyphens** — `a-z`, `0-9`, `-` (plus dots, but ONLY inside `release/` version descriptions)
- **Dots allowed only** in `release/` version descriptions (e.g., `release/v1.2.0`); any other type must not contain dots
...
Dots are only valid inside the description when the branch type is `release`; all other types use a hyphen-only description.

```abnf
branch-name       = trunk-branch / prefixed-branch
trunk-branch      = "main" / "master" / "develop"
prefixed-branch   = release-branch / other-branch
release-branch    = "release" "/" version-description
other-branch      = other-type "/" plain-description
other-type        = "feature" / "feat" / "bugfix" / "fix"
                   / "hotfix" / "chore"
                   / "ai" / "copilot" / "cursor"
                   / "claude" / "codex"
plain-description = segment *("-" segment)
version-description = ver-segment *("-" ver-segment)
ver-segment       = 1*(ALPHA / DIGIT) *("." 1*(ALPHA / DIGIT))
...
| `feature/v1.2.0` | Dots used outside `release/` type |
...
- Strip dots entirely unless the type is `release` (then keep dots only within the version number)
...
If neither approach returns a result (e.g., a brand-new repo with no commits
yet), ask the user which branch to use as the base, or default to `main`.
...
If `git pull` fails (e.g., no upstream configured, offline, or diverged
history), tell the user the pull failed and ask whether to proceed with the
local copy of `<base>` or resolve the pull issue first — do not silently
branch from a possibly stale base.
